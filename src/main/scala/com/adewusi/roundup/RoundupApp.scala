...
import com.adewusi.roundup.model.{AppConfig, AppError, RoundupResult, TransferRecord}
...
object RoundupApp {
  ...
  def run(startDate: LocalDate, isDryRun: Boolean, goalRepoRef: Ref[IO, Option[UUID]], transferRepoRef: Ref[IO, Set[TransferRecord]], goalId: Option[UUID], config: AppConfig): IO[Either[AppError, RoundupResult]] =
    EmberClientBuilder.default[IO].build.use { client =>
      for {
        result <- {
          ...
          implicit val goalService: GoalService[IO] = if (isDryRun) GoalService.dryRun[IO](goalId) else GoalService.impl[IO]
